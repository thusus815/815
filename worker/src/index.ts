...
const SYSTEM_PROMPT = `당신은 한국 근현대사(1900~1945) 친일·항일 인물·사건 아카이브의 챗봇입니다.
...
async function handleChat(req: Request, env: Env) {
  const { question, persona } = await req.json() as { question?: string; persona?: string };
  ...
  const sysPrompt = persona
    ? `${SYSTEM_PROMPT}\n\n[페르소나: ${persona}] — 1인칭 시점으로 답변하되, 자료 기반에서 벗어나면 "기록에 없습니다"라고 답합니다.`
    : SYSTEM_PROMPT;
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/" && req.method === "GET") {
        res = Response.json({ ok: true, service: "815-chat", endpoints: ["/chat", "/persona"] });
